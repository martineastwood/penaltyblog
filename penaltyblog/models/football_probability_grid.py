...
def create_dixon_coles_grid(
    ...
    """
    Creates a FootballProbabilityGrid using the Dixon-Coles adjusted Poisson distribution.

    This function is designed to decouple the prediction of expected goals from the
    subsequent market pricing calculations. It allows users to supply their own
    expected goals (lambdas) generated by external machine learning models (e.g.,
    XGBoost trained on event data) or Bayesian simulations, rather than relying on
    traditional Maximum Likelihood Estimation built into standard statistical models.

    The function computes an independent Poisson matrix and then applies the
    Dixon-Coles tau (\tau) adjustment via the `rho` parameter to correct the
    historical underestimation of low-scoring draws (0-0, 1-1, 1-0, 0-1).

    Parameters
    ----------
    home_lambda : float
        The expected number of goals for the home team (E[H]).
    away_lambda : float
        The expected number of goals for the away team (E[A]).
    rho : float, optional (default=0.0)
        The Dixon-Coles correlation parameter. Adjusts the probabilities of
        0-0, 1-0, 0-1, and 1-1 scorelines. A value of 0.0 falls back to a
        standard independent Poisson grid.
    max_goals : int, optional (default=15)
        The maximum number of goals to calculate probabilities for, defining
        the dimensions of the resulting (max_goals + 1, max_goals + 1) matrix.

    Returns
    -------
    FootballProbabilityGrid
        An initialized probability grid ready for vectorized querying of betting
        markets (1X2, Asian Handicaps, Over/Under, BTTS, etc.).

    Raises
    ------
    ValueError
        If home_lambda or away_lambda are less than or equal to zero.
    """
