...
from .utils import rho_correction
...
class RueSalvesenGoalModel:
    ...
    def __init__(self, goals_home, goals_away, teams_home, teams_away, weights=1):
        ...
        self.fixtures["goals_home"] = np.where(
            self.fixtures["goals_home"] > 5, 5, self.fixtures["goals_home"]
        )

        self.fixtures["goals_away"] = np.where(
            self.fixtures["goals_away"] > 5, 5, self.fixtures["goals_away"]
        )
    ...
    def __repr__(self):
        ...
        repr_str += "\n"
        repr_str += "Gamma: {0}".format(round(self._params[-1], 3))
        repr_str += "\n"
    ...
    def _fit(params, fixtures, teams):
        ...
        df2["dc_adj"] = df2.apply(
            lambda x: rho_correction(
                x["goals_home"], x["goals_away"], x["home_exp"], x["away_exp"], x["rho"]
            ),
            axis=1,
        )

        df2["llk"] = (
            np.log(df2["home_llk"]) + np.log(df2["away_llk"]) + np.log(df2["dc_adj"])
        ) * df2["weights"]
    ...
    def predict(self, home_team, away_team, max_goals=15):
        ...
        # get the pysological effect
        psyc_effect = (
            (home_attack + home_defence - away_attack - away_defence) / 2
        ) * gamma
...
def psychological_effect(attack_home, defence_home, attack_away, defense_away):
    pe = (attack_home + defence_home - attack_away - defense_away) / 2
    return pe
